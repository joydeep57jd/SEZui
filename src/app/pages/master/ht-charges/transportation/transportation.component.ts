...
import {TRANSPORTATION_DATA} from "./transportation-data";
...
export class TransportationComponent {
  ...
  readonly headers = DATA_TABLE_HEADERS.MASTER.HT_CHARGES.TRANSPORTATION
  readonly apiUrls = API.MASTER.HT_CHARGES.TRANSPORTATION;
  readonly applicableForList = TRANSPORTATION_DATA.applicableForList;
  readonly values = TRANSPORTATION_DATA.values;
  ...
  makeForm() {
    this.form = new FormGroup({
      transportationChargesID: new FormControl(0, []),
      effectiveDate: new FormControl(null, []),
      sacCodeId: new FormControl(null, []),
      applicableForId: new FormControl("", []),
      valueId: new FormControl("", []),
      rate: new FormControl(null, []),
      additionalRatePerPacket: new FormControl(null, []),
    })
  }
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      this.apiService.post(this.apiUrls.SAVE, data).subscribe({
        next:() => {
          this.toasterService.showSuccess("Transportation saved successfully");
          this.table.reload();
          this.makeForm();
  ...
  makePayload() {
    const applicableForName = this.applicableForList.find(applicableFor => applicableFor.value == this.form.value.applicableForId)?.label ?? '';
    return {...this.form.value, effectiveDate: this.utilService.getDateObject(this.form.value.effectiveDate), applicableForName};
  }
  ...
  setHeaderCallbacks() {
    this.headers.forEach(header => {
      ...
      if(header.field === 'value') {
        header.valueGetter = (row) => this.values.find(value => value.value === row.valueId)?.label ?? ''
      }
    });
  }
