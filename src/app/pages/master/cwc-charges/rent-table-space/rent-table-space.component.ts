...
export class RentTableSpaceComponent {
  ...
  readonly headers = DATA_TABLE_HEADERS.MASTER.CWC_CHARGES.RENT_TABLE_SPACE;
  readonly apiUrls = API.MASTER.CWC_CHARGES.RENT_TABLE_SPACE;
  ...
  makeForm() {
    this.form = new FormGroup({
      rentTableSpaceID: new FormControl(0, []),
  ...
  submit() {
    ...
    if (this.form.valid) {
      ...
      this.apiService.post(this.apiUrls.SAVE, data).subscribe({
        next:() => {
          this.toasterService.showSuccess("Rent table space saved successfully");
          this.table.reload();
          this.makeForm();
