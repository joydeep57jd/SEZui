...
export class GodownInvoiceHelper {
  ...
  async makePayload(value: any, chargeDetails: any, storageChargeDetails: any,insuranceChargeDetails: any, selectedContainerList: any[], partyList: any[]) {
    ...
    const rateApis = [
      this.apiService.get(API.CHARGE_RATE, {chargeType: CHARGE_CODE.ENTRY, sacCode: chargeDetails.entrySacCode, isImport: 'Import', ishigh: true}),
      this.apiService.get(API.CHARGE_RATE, {chargeType: CHARGE_CODE.STORAGE, sacCode: storageChargeDetails.sacCode, isImport: 'Import', ishigh: true}),
    ]

    if(Object.keys(insuranceChargeDetails).length) {
      rateApis.push(
        this.apiService.get(API.CHARGE_RATE, {chargeType: CHARGE_CODE.INSURANCE, sacCode: insuranceChargeDetails.sacCode, isImport: 'Import', ishigh: true})
      )
    }
